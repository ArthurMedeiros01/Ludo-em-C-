...
namespace q5
{
    class Tabuleiro
    {
        ...
        public int PesicaoNoTabuleiro(int idDoJogadorQueMoveu, int idDoJogadorParado, int idDoPeaoDoJogadorQueMoveu, int idDoPeaoParado)
        {
        ...
        public void VerificarCaptura(int idDoJogadorQueMoveu, int idDoPeaoDoJogadorQueMoveu)
        {
            for (int i = 0; i < jogadores.Length; i++)
            {
                for (int j = 0; j < jogadores.Length; j++)
                {
                    if (idDoJogadorQueMoveu == i)
                    {

                    }
                    else
                    {
                        if (jogadores[idDoJogadorQueMoveu].peoes[idDoPeaoDoJogadorQueMoveu].posicao == jogadores[i].peoes[j].posicao)
                        {
                            if (VerificarCasaSegura(jogadores[i].peoes[j].posicao))
                            {
                                Console.WriteLine("Seu peão não capiturou nenhum peão");
                            }
                            else
                            {
